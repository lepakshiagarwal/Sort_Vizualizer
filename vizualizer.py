...
def generate():
    ...
    for bar in range(1, 60):
        ...
        bar = canvas.create_rectangle(barstart, randomY, barend, 365, fill='grey')
        barList.append(bar)
        barstart += 10
        barend += 10
...
merge.grid(column=5,row=1)
heap.grid(column=6,row=1)
counting.grid(column=7,row=1)
shuffle.grid(column=0, row=1)
